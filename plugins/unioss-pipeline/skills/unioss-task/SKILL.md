...
`<description>` — a plain-language description of the task.

## Workflow

Use the `unioss-pipeline` skill in **task mode** (see its "Entry modes" section) and follow it exactly:

1. Derive the artifact identity `TASK#<short-slug>` from the request; open `round-1/` under `.walkthrough/TASK-<slug>/` (hyphenated folder, matching the `#`→`-` folder rule) and write `round-brief.md` from it.

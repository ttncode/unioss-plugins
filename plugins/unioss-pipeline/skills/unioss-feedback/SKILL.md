---
name: unioss-feedback
description: Continue a shipped UNIOSS ticket from customer feedback — opens a new round (does not restart A→Z).
---
...
`<gitlab-ticket-url>` — the GitLab URL of a ticket that already has ≥1 sealed round.

## Workflow

Use the `unioss-pipeline` skill in **feedback mode** (see its "Entry modes" section) and follow it exactly:
